...
async def create_project(user_id: str, title: str, prompt: str) -> Optional[dict]:
    ...
    """
    client = await get_supabase_client()
    logger.debug("Creating project: user_id=%s title=%r prompt_len=%d", user_id, title, len(prompt))
    try:
        ...
        logger.debug("Project creation %s: id=%s", "succeeded" if project else "failed", project.get("id") if project else None)
        return project
    except Exception as e:
        logger.error("Failed to create project for user %s: %s", user_id, e)
        return None
...
async def ensure_profile_exists(user_id: str) -> Optional[dict]:
    """Upsert a profiles row for user_id if one does not already exist.
    Uses ON CONFLICT DO NOTHING so it is safe to call on every request.
    Returns the profile dict.
    """
    client = await get_supabase_client()
    try:
        response = await client.table("profiles").upsert({
            "id": user_id,
        ...
        if response.data:
            logger.info("Profile ensured/created for user_id=%s", user_id)
            return response.data[0]
        return None
    except Exception as e:
        logger.error(f"Failed to ensure profile exists for user {user_id}: {e}")
        return None
