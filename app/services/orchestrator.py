...
async def run(request: GenerateUIRequest, user_id: str) -> AsyncGenerator[str, None]:
    ...
    if final_state["is_valid"]:
        try:
            ...
            # Guarantee profile row exists (auto-creates if missing)
            await supabase.ensure_profile_exists(user_id=user_id)
            logger.info("Profile ensured for user_id=%s", user_id)
